...
These conventions apply to **all** files and folders in the workspace — source, config, docs, scripts, tests, assets, and infrastructure paths. They do **not** apply to:

- **`SKILL.md`** (fixed Cursor skill filename) or other files under `cursor-config/skills/`
- **`cursor-config/rules/*.mdc`**
...
| **Files** | Lowercase **kebab-case** stem + extension: `data-extractor.md`, `docker-compose.yml`, `poll-worker.py` |
...
| `poll_worker.py` | `poll-worker.py` | kebab-case stem for source files |
...
4. After markdown path changes in Data Engineering workspaces, run `review-markdown-structure` or pre-commit.

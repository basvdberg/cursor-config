...
This reviewer targets solution and engineering documentation. It does **not** apply to:

- **`SKILL.md`** and other markdown under `cursor-config/skills/` (Cursor skill convention: YAML frontmatter first, no TOC/structure blocks)
- **`cursor-config/rules/*.mdc`** (Cursor rules)

Scripts skip these paths automatically. Do not add `## Table of contents` or `## Project structure` to skill or rule files.
...
**Exempt** everywhere: `prompts.md`, `*.handlebars.md`, `SKILL.md`, all files under `cursor-config/skills/`, `cursor-config/rules/*.mdc`, files under `data-engineering-design-patterns/design-patterns/`, `definitions/`, `implementation/`, and the design-patterns repo root `readme.md`.

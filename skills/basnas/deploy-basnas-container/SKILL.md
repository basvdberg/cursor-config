...
| `infra/` compose or `.env` on NAS | Commit and push to `main`; `deploy-on-nas.sh` syncs infra automatically via `release/deploy-config.json` ([deploy-data-solution-basnas](../deploy-data-solution-basnas/SKILL.md)) |
| New HTTPS vhost, port, or NGINX upstream | This skill (registry + templates below) |

Airflow URL: `https://airflow.example/` (inventory in [basnas-inventory.yaml](basnas-inventory.yaml)).
...
6. Run bookmark sync: `cursor-config/scripts/sync_browser_bookmarks.py --basnas` (see [browser-bookmarks-sync](../../sync/browser-bookmarks-sync/SKILL.md))

### Step 9: Browser bookmarks (Floccus)

After `service-url-map.yaml` has the canonical `url`, refresh the **local server** folder in browser-bookmarks-sync (see [browser-bookmarks-sync](../../sync/browser-bookmarks-sync/SKILL.md)):
...
- [basnas-inventory.yaml](basnas-inventory.yaml) · [service-url-map.yaml](service-url-map.yaml) · [browser-bookmarks-sync](../../sync/browser-bookmarks-sync/SKILL.md)
